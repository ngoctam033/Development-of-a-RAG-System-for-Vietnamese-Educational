...
# Updated imports following the new project structure
...
def get_all_files_in_folder(
    ...
    # Log results
    if len(files_info) > 0:
        logger.info(f"Found {len(files_info)} {', '.join(extensions)} files in {folder_path}")
    else:
        logger.warning(f"No {', '.join(extensions)} files found in {folder_path}")
...
def chunk_markdown_file(input_file_path: str, output_file_path: str = "vectorization_data.json") -> List[Dict[str, any]]:
    ...
    # Step 3: Flatten hierarchy into chunks
    ...
    # Step 4: Post-process chunks if needed
    ...
    if not save_success:
        logger.warning("Error saving file, but chunks were returned.")

    logger.info(f"Chunking pipeline complete: {len(processed_chunks)} chunks generated.")
...
def main() -> None:
    ...
    logger.info(f"\n📄 STEP 1: PROCESSING {len(files)} PDF FILES")
    logger.info("-" * 40)
    ...
    if not all_chunks:
        logger.error("❌ No chunks were created. Pipeline halted.")
        return
    ...
    # Add index to chunks for tracking
    ...
    if vectorization_result["success"]:
        ...
        logger.error(f"❌ VECTORIZATION FAILED: {vectorization_result.get('error', 'Unknown error')}")
    ...
    if vectorization_result["success"]:
        ...
        logger.info(f"   🚀 System ready for Q&A!")
    else:
